...
m_df = m_df.groupby(['gwas_id', '_merge'], observed=False).size().reset_index()
m_df = m_df.loc[m_df['_merge'] != 'right_only']
m_df = m_df.pivot_table(index=['gwas_id'], columns=['_merge'], values=0, observed=False)
...
# m_df['loci_explained_proportion'] = m_df['explained_cnt'] / m_df['loci_cnt'] * 100
m_df['loci_explained_proportion'] = m_df['explained_cnt'] / m_df['loci_cnt']
# m_df['loci_explained_proportion'] = m_df['loci_explained_proportion'].apply(int)
m_df['loci_explained_proportion'] = m_df['loci_explained_proportion']
...
m_df = m_df[['gwas_id', 'gwas_trait', 'gwas_trait_ontology_term', 'gwas_category_ontology_term', 'loci_cnt', 'explained_cnt', 'loci_explained_proportion']]
...
agg_data = m_df.groupby('gwas_category_ontology_term', observed=False).agg({'loci_explained_proportion': 'mean'}).sort_values('loci_explained_proportion', ascending=False)

plt.figure(figsize=(6.4, 6.4))
g = seaborn.barplot(data=m_df, y="gwas_category_ontology_term", x="loci_explained_proportion", orient='h', estimator=numpy.mean, order=agg_data.index, color='gray')
tick_fontsize2 = 16
plt.title('Explained loci', fontsize=tick_fontsize2)
plt.xlabel('Proportion', fontsize=tick_fontsize2)
plt.ylabel('Trait category', fontsize=tick_fontsize2)

plt.tight_layout()
png_path = os.path.join(outdir_path, "loci_explained_proportion.png")
...
for gwas_category_ontology_term in sorted(m_df['gwas_category_ontology_term'].unique()):
    ...
    # import pdb; pdb.set_trace()
    m2_df = m2_df.replace([numpy.inf, -numpy.inf], numpy.nan)  # Replace inf and -inf with NaN
    g = seaborn.stripplot(ax=axes[int(counter/4), counter%4], data=m2_df, y="gwas_trait_ontology_term", x="loci_explained_proportion", orient='h', order=order, s=5)
    # import pdb; pdb.set_trace()
    g.tick_params(axis='y', which='major', labelsize=8)
    g.set(ylabel=None)
    g.set(xlabel="Proportion explained")
