...
try:
    ...
    tabix_ftp_paths_url = sys.argv[4]
    coloc_path_strf = sys.argv[5]
    if len(sys.argv) > 6:
...
eqtl_df = pandas.read_csv(tabix_ftp_paths_url, sep="\t")
# eqtl_df = eqtl_df.query('quant_method=="ge" | quant_method=="microarray"')
...
for eqtl_id in eqtl_df.index.tolist():
    ...
    match = re.search(r'(.+)/\{gwas_id\}', coloc_path_strf)
