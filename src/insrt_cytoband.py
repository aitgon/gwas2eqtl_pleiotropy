...
from gwas2eqtl_pleiotropy_db import Base, cytoband
...
if sqlalchemy.inspect(engine).has_table(cytoband.__tablename__):
    cytoband.__table__.drop(engine)
Base.metadata.tables[cytoband.__tablename__].create(bind=engine)
...
with engine.connect() as connection:
    connection.execute((Base.metadata.tables['cytoband']).delete())
