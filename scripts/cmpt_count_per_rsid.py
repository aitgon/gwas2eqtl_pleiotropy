...
#%% etissue pleiotropy
etissue_df = coloc_df[['chrom', 'cytoband', 'pos38', 'rsid', 'etissue_class']].drop_duplicates().groupby(['chrom', 'cytoband', 'pos38', 'rsid']).agg({'etissue_class': ['size', (lambda x: (",".join(sorted(x))))]}).reset_index()
etissue_df.columns = ['chrom', 'cytoband', 'pos38', 'rsid', 'etissue_label_count', 'etissue_class_lst']
etissue_df = etissue_df.sort_values(by=['etissue_label_count', 'chrom', 'pos38', 'etissue_class_lst', 'rsid'], ascending=[False, True, True, True, True])
...
#%% gwas pleiotropy
gwas_df = coloc_df[['chrom', 'cytoband', 'pos38', 'rsid', 'gwas_class']].drop_duplicates().groupby(['chrom', 'cytoband', 'pos38', 'rsid']).agg({'gwas_class': ['size', (lambda x: (",".join(sorted(x))))]}).reset_index()
gwas_df.columns = ['chrom', 'cytoband', 'pos38', 'rsid', 'gwas_class_count', 'gwas_class_lst']
gwas_df = gwas_df.sort_values(by=['gwas_class_count', 'chrom', 'pos38', 'gwas_class_lst', 'rsid'], ascending=[False, True, True, True, True])
...
m_df = m_df[['chrom', 'cytoband', 'pos38', 'rsid', 'gwas_class_count', 'gwas_class_lst', 'egene_count', 'eqtl_gene_symbol_lst', 'etissue_label_count', 'etissue_class_lst', 'egene_lst']]
m_df.sort_values(['gwas_class_count', 'chrom', 'pos38', 'rsid'], ascending=[False, True, True, True], inplace=True)
