...
%
These filters resulted in 335 GWAS with 10,627 clumped leading variants and a p-value below 5e-8 (Supplementary table 2).
% sqlite
...
The largests classes of diseases by number of leading GWAS variants are autoimmune diseases (2107), cancer of breast (1976) and allergy (1321)
%
and the percentage of variants with a colocalized eQTL is 37\%, 21\% and 42\%, respectively (Supplementary table ST6).
%
...
%
For the autoimmune diseases, this is in agreement with previous work \citep{2021.Li.Mu.GenomeBiology.impactcelltype}.

For the autoimmune diseases, ? of the ? leading variants had an annotation with probability PP.H4.abf$\ge$0.8.
% sqlite
% select count(distinct tophits.rsid) from tophits, coloc, gwas_annot where gwas_annot.gwas_class='Autoimmune dis.' and gwas_annot.gwas_id=tophits.gwas_id and coloc.rsid=tophits.rsid and coloc.`PP.H4.abf`>=0.8;
% 778
% grouped H4 0.8: select count(distinct tophits.rsid) from tophits, coloc, gwas_annot where gwas_annot.gwas_id=tophits.gwas_id and coloc.rsid=tophits.rsid and coloc.`PP.H4.abf`>=0.8 GROUP by  gwas_annot.gwas_class;
%
This is perc of the
% grouped all: select count(distinct tophits.rsid) from tophits, coloc, gwas_annot where gwas_annot.gwas_id=tophits.gwas_id and coloc.rsid=tophits.rsid GROUP by  gwas_annot.gwas_class;%
%
...
This pipeline has resulted in 103,551 variants with or from 246 GWAS that colocalized (PP.H4.abf 0.8) with at least one eQTL.
%select count(*) from (select DISTINCT gwas_id FROM coloc where `PP.H4.abf`>=0.8);
%
This is a useful annotation ressource that I have made available through a web application (TODO).
%
%This analysis tested 139,069 unique variants in windows of one million nucleotides around the GWAS leading variants and ? potential variant/GWAS/eQTL sample and e-gene colocalizations from 418 GWAS studies (OSF URL).
%select count(*) from (select DISTINCT chrom,pos,rsid FROM coloc); 139,069
%select count(*) from (select DISTINCT chrom,pos,rsid,gwas_id,eqtl_id,egene FROM coloc); 17,595,868
%
%There are 103,551 variants from 246 GWAS with a least one colocalized eQTL and probability PP.H4.abf$\ge$0.8.
%select count(DISTINCT gwas_id) from coloc where `PP.H4.abf`>=0.8; 246
%select count(*) from (select DISTINCT chrom,pos,rsid FROM coloc where `PP.H4.abf`>=0.8); 103,551
%
%select count(*) from (select DISTINCT chrom,pos,rsid,gwas_id,eqtl_id,egene FROM coloc where `PP.H4.abf`>=0.8); 2,477,640
...
%
Therefore I manually defined 36 categories to annotate eQTL samples at equivalent resolution (Supplementary table 1).
...
%
For instance the variant rs2522051, which is located downstream of the IRF1 gene,

re located upstream and downstream of the IRF1 gene, respectively.
...
Pleiotropic variants aggregate in some cytobands such as 5q31.1, 19q13.33, 3q23, 9p21.3 and 15q24.1 (Table \ref{tab:pleitropic_variants} and Supplementary table 3).
