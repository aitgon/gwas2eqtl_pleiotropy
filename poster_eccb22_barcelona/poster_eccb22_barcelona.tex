	...
	\begin{minipage}[c][10cm][c]{0.1\textwidth}
	  \begin{center}
	    \includegraphics[width=12cm,angle=0]{img_tmp/logo_sciences.eps}
	  \end{center}
	\end{minipage}
	...
	\begin{minipage}[c][10cm][c]{0.1\textwidth}
	  \begin{center}
	    \includegraphics[width=12cm,angle=0]{img_tmp/logo_tagc_400x230_centered_1.eps}
	    \includegraphics[width=12cm,angle=0]{img_tmp/logo_tagc_400x230_centered_1.eps}
	  \end{center}
	\end{minipage}
	} % end pbox
	...
	\begin{minipage}{1\columnwidth}
	    \begin{center}
		    ...
		    \end{minipage}
		    \mycaptionfig{Count of eGenes per variant-tissue (\textbf{Left}), eTissues per variant-eGene (\textbf{Center}) and GWAS categories per variant-eGene-eTissue (\textbf{Right}).}
	    \end{center}
	\end{minipage}
	...
	\begin{minipage}{1\columnwidth}
	    \begin{center}
		    ...
		    \end{minipage}
		    \mycaptionfig{Variant effect predictor (VEP) analysis as a function of the number of GWAS catategory number for three different consequences: Missense variant (Left), splice region variant (Center) and 3’ UTR variant (Right)}
	    \end{center}
	\end{minipage}
	...
	three possible mechanisms of pleiotropy. Left, Pleiotropic variants have more
	...
	Center, I have also found that eGenes of pleiotropic variants are active
	in more etissues which result in more GWAS phenotypes. This might be
	explained from variants being bound by more transcription factors. Left
	...
	\item A pipeline to colocalize eQTLs and GWAS variantsWe present a new method to learn a supervised model of associated SNPs based on regulatory features (Fig. 1).
